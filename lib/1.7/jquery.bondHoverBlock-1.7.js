...
(function ($) {
    $.fn.bondHoverBlock = function (options) {
        var defaults = {
            ...
            opacityInit: null,  /*прозрачность блока при инициализации*/
        ...
        function init() {
            var pos_css_top = (this$.css("top") == "auto") ? "" : parseInt( this$.css("top"), 10 ) ;
            if ( this$.css("position") != "fixed" ) {
                memory.positionBlock = getOffset(this$.get(0));
            }
            else {
                memory.positionBlock = {
                    top: pos_css_top,
                    left: null
                }
            }
            this$.data({"top":pos_css_top });
            this$.data({"opacity": (this$.css("opacity")==1)? 1: this$.css("opacity") });
            if (options.opacityInit != null)
                this$.data({"opacity": options.opacityInit });
            if (options.isCopy) {
        ...
        function bindEvent() {
            $(window).scroll(function (e) {
                ...
                if ( (windowScrollTop - options.topPosWindow) >= ( memory.positionBlock.top - options.topPosblock) ) {
                    /* проскролили и надо показывать блок */
                    if (!memory.isFix || memory.isFix == null ) {
                        this$.css({opacity:0});
                    ...
                    if (memory.isFix || memory.isFix == null ) {
                        this$.stop().animate({opacity:0}, options.hideSpeed, "linear", function () {
                            ...
                            if (options.isCopy && memory.isFix != null) {
                                this$.unwrap();
                                anyfunc();
                            }
                        });
            ...
            if (options.opacityMouseOver) {
                this$.mouseenter(function (e) {
                    if (memory.isFix) {
                        var localOpacity = (options.opacityShow) ? options.opacityShow : this$.data("opacity");
                        this$.stop().animate({opacity:localOpacity}, options.opacityShowSpeed, "linear");
                    }
                });
